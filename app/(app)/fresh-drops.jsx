...
const SIGNALS = [
  { icon: 'cube-outline',    label: 'Total Listings', color: '#D97706', bg: '#FFFBEB' },
  { icon: 'trending-up',     label: 'Categories',     color: '#059669', bg: '#ECFDF5', value: '18+' },
  { icon: 'cash-outline',    label: 'Avg Value',      color: '#2563EB', bg: '#EFF6FF', value: '₦85k' },
  { icon: 'people-outline',  label: 'Active Traders', color: '#7C3AED', bg: '#F5F3FF', value: '24k+' },
];

const STEPS = [
  { icon: 'notifications',   color: '#D97706', bg: '#FFFBEB', num: '01', title: 'Items Land Instantly',  desc: 'Every listing posted on SwapNaija appears here the moment it goes live — no curation, no delay.' },
  { icon: 'time-outline',    color: '#2563EB', bg: '#EFF6FF', num: '02', title: 'Sorted by Recency',      desc: 'The default "Just Dropped" sort puts the newest listings first so you never miss a fresh item.' },
  { icon: 'filter-outline',  color: '#059669', bg: '#ECFDF5', num: '03', title: 'Filter by Category',     desc: 'Use the category chips to narrow down to electronics, fashion, agric items, services, and more.' },
  { icon: 'refresh',         color: '#7C3AED', bg: '#F5F3FF', num: '04', title: 'Updated in Real Time',   desc: 'Refresh at any time to see the very latest drops. Listings update as traders post them.' },
];
    ...
    >
      <View style={lc.imgWrap}>
        ...
        <View style={lc.newBadge}>
          ...
          <Text style={lc.newText}>New</Text>
        </View>
        {listing.condition && (
  ...
  return (
    <View style={h.card}>
      ...
      <View style={h.badge}>
        ...
        <Text style={h.badgeText}>Real-time feed · Updated as listings go live</Text>
      </View>
      <Text style={h.title}>Fresh{'\n'}Drops</Text>
      <Text style={h.sub}>Every new listing on SwapNaija, the moment it hits. Browse by category, sort by value, and be first to propose.</Text>
      <View style={h.statsRow}>
        {[
          { val: total ? `${total}` : '—', label: 'Listings' },
          { val: 'Real-time',              label: 'Feed Updates' },
          { val: '18+',                    label: 'Categories' },
          { val: '24,000+',               label: 'Traders' },
        ].map(({ val, label }) => (
...
export default function FreshDropsScreen() {
  const router = useRouter();
  ...
  const { data, isLoading, isFetching, refetch } = useQuery({
    ...
    queryFn: () => searchListings({ sort, page, limit: LIMIT, categoryId: categoryId || undefined }),
    ...
    <View style={s.listHeader}>
      ...
      <View style={s.howCard}>
        <View style={s.howHead}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.howTitle}>How Fresh Drops Works</Text>
            <Text style={s.howSub}>Real-time listing feed</Text>
          </View>
        </View>
        <View style={s.steps}>
          {STEPS.map((step) => (
        ...
        <View style={s.howFooter}>
          ...
          <Text style={s.howFooterText}>All listings are from SwapNaija traders. Report suspicious listings using the flag button.</Text>
        </View>
      </View>
      ...
      <View style={s.controls}>
        <View>
          <Text style={s.controlsTitle}>Latest Listings</Text>
          {!isLoading && <Text style={s.controlsSub}>{total} listings available</Text>}
        </View>
        <View style={s.controlsRight}>
          <TouchableOpacity style={s.sortBtn} onPress={() => setShowSort(v => !v)} activeOpacity={0.75}>
            <Ionicons name="swap-vertical" size={13} color="#059669" />
            <Text style={[s.sortBtnText, { color: '#059669' }]}>{activeSortLabel}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[s.refreshBtn, { backgroundColor: '#ECFDF5', borderColor: '#A7F3D0' }]} onPress={() => { setPage(1); setAllListings([]); refetch(); }} activeOpacity={0.75}>
        ...
        <FlatList
          ...
          ListFooterComponent={
            <View>
              ...
              <View style={[s.tipCard, { backgroundColor: '#FFFBEB', borderColor: '#FDE68A' }]}>
                <Ionicons name="flash" size={18} color="#D97706" />
                <View style={{ flex: 1 }}>
                  <Text style={[s.tipTitle, { color: '#92400E' }]}>Check back daily for new drops</Text>
                  <Text style={[s.tipDesc, { color: '#B45309' }]}>SwapNaija traders post hundreds of new listings every day. Don't miss an item in your favourite category.</Text>
                </View>
              </View>
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              <View style={s.emptyIcon}><Ionicons name="flash-outline" size={32} color={COLORS.textLight} /></View>
              <Text style={s.emptyTitle}>No fresh drops yet</Text>
              <Text style={s.emptySub}>Be the first to list something amazing!</Text>
              <TouchableOpacity style={[s.emptyAction, { backgroundColor: COLORS.primaryLight, borderColor: `${COLORS.primary}30` }]} onPress={() => router.push('/(app)/create-listing')} activeOpacity={0.8}>
                <Text style={[s.emptyActionText, { color: COLORS.primary }]}>Create a Listing</Text>
              </TouchableOpacity>
            </View>
          }
        />
...
const h = StyleSheet.create({
  card: { backgroundColor: '#051a10', padding: 20, overflow: 'hidden', marginBottom: 14 },
  ring: { position: 'absolute', borderRadius: 999, borderWidth: 1, borderColor: 'rgba(16,185,129,0.2)' },
...
const lc = StyleSheet.create({
  ...
  newBadge: { position: 'absolute', top: 7, left: 7, flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: COLORS.primary, paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8 },
  newText: { fontSize: 9, fontWeight: '800', color: '#fff' },
...
const s = StyleSheet.create({
  ...
  sortBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#ECFDF5', paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20, borderWidth: 1, borderColor: '#A7F3D0' },
  sortBtnText: { fontSize: 12, fontWeight: '700' },
  refreshBtn: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
  ...
  tipCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, marginHorizontal: 16, marginBottom: 8, borderRadius: 18, padding: 14, borderWidth: 1 },
  ...
  emptyAction: { borderRadius: 14, paddingHorizontal: 18, paddingVertical: 10, borderWidth: 1, marginTop: 4 },
